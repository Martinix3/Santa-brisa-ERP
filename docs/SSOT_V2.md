# SSOT V2.1 - ESPECIFICACIÓN TÉCNICA COMPLETA
## Sistema de Inventario + Quality + Compliance "Enterprise-Grade"

**Versión:** 2.1.0  
**Fecha:** 20 de Octubre 2025  
**Changelog:** Añadidas extensiones Quality+ y Compliance como colecciones canónicas  

### RESUMEN EJECUTIVO

Esta especificación implementa el SSOT v2.1 con:
- ✅ Transaccionalidad completa
- ✅ Buckets QC (RELEASED/HOLD/REJECTED)
- ✅ Generación race-free de lotCode
- ✅ **Quality module integrado** (planes, releases, NC)
- ✅ **Compliance/PRP** (protocolos regulatorios)
- ✅ **Documents v2** (versionado, aprobaciones, OCR)
- ✅ **Gemini IA** (reemplaza alertEvents)

**Total colecciones canónicas:** 19 (vs 6 en v2.0)

---

## 0. DECISIONES ARQUITECTÓNICAS FINALES

### 0.1 REFERENCIAS CANÓNICAS (AMPLIADAS v2.1)
...
- **Lote canónico:** `lotCode` (YYJJJ-PL[-LN]-SEQ), independiente del SKU
  - Formato mejorado: `^\d{5}-[A-Z0-9]+(?:-[A-Z0-9]+)?-\d{3}$` (soporta multi-segment)
- **Ubicaciones:** `locations` + `fromLocationId`/`toLocationId` siempre
- **Dinero:** Enteros en céntimos (`amountCents`) + `currency`

**Quality & Compliance (NUEVO v2.1):**
- **Parámetros QC:** `parameterId` (NO inline parameters)
- **Métodos:** `methodId` (catálogo centralizado)
- **Protocolos:** `protocolId`, `protocolRunId`
- **Documentos:** `linkedEntity.type` ampliado (incluye `protocol`, `protocolRun`)
- **Tasks:** `linkedEntity` tipado para acciones cross-módulo

### 0.2 SEPARACIÓN DE COLECCIONES (ACTUALIZADA v2.1)

**Dominio vs Monitorización:**
- **traceEvents** (dominio) ≠ **geminiAnalyses** (monitoring) ← CAMBIO: reemplaza alertEvents
- **documents v2** única colección con versionado + aprobaciones + OCR
- **onHand** con buckets QC (RELEASED/HOLD/REJECTED)

**Quality:**
- **qualityPlans** - Definición de tests por scope
- **qualityReleases** - Decisiones QC con resultados
- **nonConformances** - NC como entidad de dominio

**Compliance/PRP:**
- **productionProtocols** - Checklists inteligentes (6 step types)
- **productionProtocolRuns** - Ejecuciones con evidencias
- **complianceSchedule** - Programación automática

**Catalogos:**
- **analysisMethods** - Métodos de ensayo
- **analysisParameters** - Parámetros medibles

**System:**
- **geminiAnalyses** - Alertas IA (reemplaza alertEvents)
- **tasks** - Pegamento inter-módulos
- **auditLogs** - Log inmutable con hash chain

### 0.3 EXTENSIONES SSOT V2.1 (NUEVAS)

**Módulos Canónicos Añadidos:**

1. **Quality Module** (5 colecciones)
   - Gestión completa de QC con trazabilidad
   - Librería de métodos/parámetros reutilizables
   - Non-conformances como entidad de dominio

2. **Compliance/PRP** (3 colecciones)
   - Protocolos regulatorios (Plagas, Aguas, Limpieza, Formación, etc.)
   - Programación automática con recordatorios
   - 6 tipos de steps: CHECK, MEASURE, VERIFY_DOC, PHOTO, SIGN, INPUT

3. **Documents v2** (1 colección actualizada)
   - Versionado automático
   - Workflow de aprobaciones con firmas
   - OCR integrado
   - Vigencia temporal

4. **Gemini & Tasks** (2 colecciones)
   - Separación dominio/monitorización
   - Auto-acciones (CREATE_TASK, SEND_EMAIL, OPEN_DRAWER)
   - Tasks como pegamento entre módulos

5. **Audit & Observability** (1 colección)
...
- `SSOT_V2_PLUS_IMPLEMENTATION_READY.md` (MASTER)
