...
export default function PagosPage() {
  ...
  const paymentsWithDetails = useMemo(() => {
    return paymentLinks.map(payment => {
      ...
      new Date(b.paidAt ?? b.date ?? Date.now()).getTime() - new Date(a.paidAt ?? a.date ?? Date.now()).getTime()
    );
  ...
  const stats = useMemo(() => {
    ...
    const thisMonthPayments = paymentLinks.filter(p => {
      const date = new Date(p.paidAt ?? p.date ?? Date.now());
                      ...
                      <tr key={payment.id}>
                        ...
                        <td>
                          <div className="flex items-center gap-2">
                            ...
                            {new Date(payment.paidAt ?? payment.date ?? Date.now()).toLocaleDateString('es-ES')}
                          </div>
                        </td>
