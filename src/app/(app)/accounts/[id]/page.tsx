...
export default function AccountDetailPage() {
  ...
  const loadData = React.useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [a, s, o, act, n, cp, alts] = await Promise.all([
        getAccount(id),
  ...
  if (loading) {
    return (
      <div className="p-6 text-sm text-muted-foreground">Cargando cuenta...</div>
    );
  }

  if (!account) {
    return (
      <div className="p-6 text-muted-foreground">Cuenta no encontrada</div>
    );
  }
    ...
    <div className="p-4 md:p-6 space-y-5">
      ...
      <div className="sb-header-glass p-5 md:p-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="space-y-2 flex-1 min-w-0">
            <h1 className="text-2xl md:text-3xl font-bold truncate">{account.name}</h1>
            <div className="flex flex-wrap items-center gap-2 text-sm">
              ...
              {account.accountChannel === 'COLOCACION' ? (
                <span className="sb-kpi-badge px-3 py-1 text-xs font-medium text-blue-600 border-blue-200">
                  Colocación{account.distributorName && ` · ${account.distributorName}`}
                </span>
              ) : (
                <span className="sb-kpi-badge px-3 py-1 text-xs font-medium">
                  Venta directa
      ...
      {alerts.length > 0 && (
        <BusinessAlertsCard
          alerts={alerts}
          onActionClick={(alert) => {
            // Open QuickLog drawer for actionable alerts
          ...
          <div className="sb-card-glass-subtle p-5">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div>
                ...
                <div className="font-medium">{account.phone || account.mobile || '—'}</div>
              </div>
              {account.addr && (account.addr.street || account.addr.city) && (
                <div>
                  <div className="text-xs text-muted-foreground mb-1">Dirección</div>
                  <div className="font-medium text-xs leading-relaxed">
                    {account.addr.street || ''}
                    {account.addr.postalCode && `, ${account.addr.postalCode}`}
                    {account.addr.city && ` ${account.addr.city}`}
                    {account.addr.province && `, ${account.addr.province}`}
                  </div>
                </div>
              )}
            </div>
      ...
      <EditAccountDialog
        ...
        account={account ? {
          ...
          fiscalId: account.fiscalId || undefined,
          segment: (account.segment || 'HORECA') as any,
          flow: account.accountChannel === 'COLOCACION' ? 'PLACEMENT' : 'DIRECT',
          stage: account.stage as any,
