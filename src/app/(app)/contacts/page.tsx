    ...
    const filteredParties = useMemo(() => {
        ...
        return parties.filter(party => {
            ...
            const matchesSearch = !searchTerm ||
                (party.name || party.legalName).toLowerCase().includes(lowerCaseSearch) ||
                (party.taxId || '').toLowerCase().includes(lowerCaseSearch) ||
                (party.emails ?? []).some(e => e.value.toLowerCase().includes(lowerCaseSearch)) ||
                (party.phones ?? []).some(p => p.value.toLowerCase().includes(lowerCaseSearch));
            ...
            <SBCard title="">
                <div className="divide-y divide-zinc-100">
                    ...
                    {filteredParties.map(party => {
                        const roles = partyRolesByPartyId.get(party.id) || [];
                        const primaryContact = (party.emails || []).find(c => c.isPrimary) || (party.phones || []).find(c => c.isPrimary);
