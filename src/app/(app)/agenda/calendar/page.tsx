...
function mapInteractionsToTasks(
  ...
  return interactions
    ...
    .map((i) => {
      ...
      } as unknown as Task; // Cast as Task to satisfy the type checker after spreading properties
    })
...
function AgendaPageContent() {
  ...
  const handleDrop = async (e: DropArg) => {
      ...
      toast.success(`Tarea "${task.note}" reagendada al ${new Date(newDate).toLocaleDateString()}`);
      ...
      try {
          ...
          setData(prev => {
              if (!prev) return prev;
              const interactions = prev.interactions.map(t =>
                  t.id === task.id ? { ...t, plannedFor: (task as any).date } : t // Revert to original date
              );
              return { ...prev, interactions };
          });
      }
  ...
  // FullCalendar lazy
  const FullCalendarComponent = useMemo(() => dynamic(() => import("@fullcalendar/react"), { ssr: false }), []);
    ...
    <>
      ...
      {(modal.type === "new" || modal.type === "edit") && (
        <NewEventDialog
          ...
          dept={modal.type === "edit" ? (modal.data.dept || 'VENTAS') : 'VENTAS'}
        />
      )}
