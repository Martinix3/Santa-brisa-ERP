...
export async function updateUser(
  ...
  try {
    ...
    const userIndex = santaData.users.findIndex((u: User) => u.id === userId);
    if (userIndex === -1) {
    ...
    if (updates.email && updates.email !== currentUser.email) {
      const existingUser = santaData.users.find(
        (u: User) => u.id !== userId && u.email?.toLowerCase() === updates.email?.toLowerCase()
      );
      if (existingUser) {
        return { success: false, error: "Ya existe un usuario con ese email" };
      }
...
export async function getUserById(userId: string): Promise<{
  ...
  try {
    ...
    const user = santaData.users.find((u: User) => u.id === userId);
    if (!user) {
      return { success: false, error: "Usuario no encontrado" };
    }
...
export async function bulkUpdateKPIs(
  ...
  try {
    ...
    for (const update of updates) {
      const user = santaData.users.find((u: User) => u.id === update.userId);
      if (user) {
        usersToUpdate.push({
          ...user,
