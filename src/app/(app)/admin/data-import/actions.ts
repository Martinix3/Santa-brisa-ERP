...
function buildRegistry(data: SantaData): FKRegistry {
  ...
  const ordersSellOut = data.ordersSellOut ?? [];
  
  return {
    accountsById: new Map(accounts.map((a: Account)=>[a.id,a])),
    accountsByName: new Map(accounts.map((a: Account)=>[a.name.toLowerCase(),a])),
    accountsByCode: new Map(accounts.filter((a: Account)=>a.code).map((a: Account)=>[String(a.code).toLowerCase(),a])),
    usersById: new Map(users.map((u: User)=>[u.id,u])),
    usersByEmail: new Map(users.filter((u: User)=>u.email).map((u: User)=>[String(u.email).toLowerCase(),u])),
    itemsById: new Map(items.map((m: Item)=>[m.id,m])),
    onHandById: new Map(onHand.map(l=>[l.id,l])),
    ordersById: new Map(ordersSellOut.map(o=>[o.id,o])),
  };
}
  ...
  for (const r of rows){ const row = { ...r }; for (const k of Object.keys(row)) if (typeof row[k]==='string') row[k] = row[k].trim();
    ...
    if (coll==='goodsReceipts'){
      ...
      if (Array.isArray(lines)){
        row.lines = lines.map((ln:any)=> ({ itemId: reg.itemsById.get((ln.sku || ln.itemId))?.id ?? ln.itemId, qty: nNumComma(ln.qty), uom: ln.uom ?? 'unit' }));
      }
      out.push(row); continue;
    }
