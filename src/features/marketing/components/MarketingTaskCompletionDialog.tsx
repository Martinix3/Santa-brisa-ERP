...
import { Euro, Users, Target, BarChart3, MessageCircle, Heart, Share2, ThumbsUp } from 'lucide-react';

export function MarketingTaskCompletionDialog({ task, open, onClose, onComplete }: {
    task: Interaction;
    open: boolean;
    onClose: () => void;
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        {kpiFields.map(({label, value, setter, icon: Icon}) => (
                           <label key={label} className="grid gap-1.5">
                               <span className="text-sm font-medium text-zinc-700 flex items-center gap-1.5"><Icon size={14}/> {label}</span>
                               <input
                                   type="number"
                                   value={value || ''}
