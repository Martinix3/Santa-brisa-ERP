        ...
        <SBCard title={`Próximas Tareas de ${DEPT_META[department].label}`}>
            <div className="p-2 space-y-1">
                {allEvents.map((event: Interaction) => {
                    ...
                    return (
                         <Link href="/agenda/calendar" key={event.id} className={`flex items-center gap-3 p-2 rounded-lg border cursor-pointer ${isOverdue ? 'bg-rose-50/50 border-rose-200' : 'bg-zinc-50 border-zinc-200 hover:bg-zinc-100'}`}>
                            <div className="p-2 rounded-full" style={{ backgroundColor: `${DEPT_META[department].color}22`, color: DEPT_META[department].color }}>
                                <Icon size={16} />
                            </div>
                            <div>
