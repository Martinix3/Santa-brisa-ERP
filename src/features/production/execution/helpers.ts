...
export function picksToRealLines(picks: Array<{sku:string; lotNumber:string; qty:number; uom:string; locationId: string}>, itemsMap: Map<string, Item>): RealConsumptionLine[] {
  const bucket = new Map<string, RealConsumptionLine>();
  for (const p of picks) {
    const k = `${p.sku}|${p.lotNumber}|${p.uom}`;
    const cur = bucket.get(k) ?? { sku: p.sku, itemName: itemsMap.get(p.sku)?.name || p.sku, lotNumber: p.lotNumber, theoreticalQty: 0, realQty: 0, uom: p.uom as Uom, fromLocationId: p.locationId };
