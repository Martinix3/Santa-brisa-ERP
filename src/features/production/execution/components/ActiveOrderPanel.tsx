...
import { Play, Pause, CheckCircle, XCircle, Calendar } from "lucide-react";
...
import { canEditPlan, canStart, canPause, canResume, canFinish, isClosedLike, picksToRealLines } from '../helpers';
...
export function ActiveOrderPanel({ activeForm, setActiveForm, onProgram, items, onHand, recipes }: {
  ...
  const itemsIndex = useMemo(() => {
    ...
    for (const item of items) {
      ...
      if ((item as any).sku) map.set((item as any).sku, item);
    }
    return map;
  }, [items]);
  ...
  const requiredLotsOut = useCallback((r: any[]) => {
    setActiveForm((prev: any) => {
      if (!prev) return prev;
      const autoLines = picksToRealLines(r, itemsIndex);
      const currentLines: any[] = prev.realConsumption || [];
      const shouldPrime =
        currentLines.length === 0 ||
        currentLines.every((line) => Number(line.realQty ?? 0) === Number(line.theoreticalQty ?? 0));
      if (!shouldPrime) {
        const missingLines = autoLines.filter(
          (auto) => !currentLines.some((line) => line.sku === auto.sku && line.lotNumber === auto.lotNumber)
        );
        const merged = missingLines.length ? [...currentLines, ...missingLines] : currentLines;
        return {
          ...prev,
          requiredLots: r,
          realConsumption: merged,
        };
      }
      return {
        ...prev,
        requiredLots: r,
        realConsumption: shouldPrime ? autoLines : currentLines,
      };
    });
  }, [itemsIndex, setActiveForm]);
    ...
    startTransition(async () => {
      try {
        const res = await completeProductionOrder({
          ...
          finalOutputs: [{
            sku: activeForm.finalOutput.sku,
      ...
      const totalCost = activeForm.realConsumption.reduce((sum: number, c: any) => {
        const item = itemsIndex.get(c.sku);
      ...
      toast.success(
        <div className="space-y-3">
          ...
          <div className="text-xs space-y-1 border-t pt-2">
            ...
            <p className="pl-3">• Producto: {itemsIndex.get(activeForm.finalOutput.sku)?.name || itemsIndex.get(activeForm.finalOutput.sku)?.sku || activeForm.finalOutput.sku || 'N/A'}</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <Input 
                            type="date" 
                            className="w-full" 
                            value={activeForm?.order?.scheduledFor?.slice(0, 10) || activeForm.scheduledFor || new Date().toISOString().slice(0, 10)} 
                            ...
                            onChange={e => {
                                if (activeForm.order) {
                                    ...
                                    // Si es planificación, actualizar scheduledFor en el form
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div className="space-y-2">
                                ...
                                <div className="text-sm space-y-1 bg-secondary/30 p-3 rounded">
                                  <p><span className="font-medium">Producto:</span> {itemsIndex.get(activeForm.finalOutput.sku)?.name || (itemsIndex.get(activeForm.finalOutput.sku) as any)?.sku || activeForm.finalOutput.sku || 'N/A'}</p>
