...
export function StockCheckPanel({ bom, qty, items, onHand, onReadyChange, shortagesOut, requiredLotsOut }: {
  ...
  const { shortages, picks } = useMemo(() => {
    ...
    for (const r of onHand) {
      const sku = r.sku;
      if (!sku) continue; // Skip records without SKU
    ...
    for (const t of theory) {
      ...
      for (const r of lots) {
        ...
        // Calcular qty disponible usando SSOT v2 (buckets) o legacy
        let availableQty = 0;
        if (typeof r.qty === 'object' && r.qty !== null) {
          // SSOT v2: usar qty.RELEASED
          availableQty = (r.qty as any).RELEASED || 0;
        } else {
          // Legacy: qty as number
          availableQty = Number(r.qty) || 0;
        }
        
        const take = Math.min(availableQty, remain);
        // SSOT v2: usar lotCode canónico o fallback a lotNumbers legacy
        const lotCode = r.lotCode || (r.lotNumbers ? Object.keys(r.lotNumbers)[0] : '');
        const locationId = r.locationId || r.warehouseId || '';
        if (take > 0 && lotCode && locationId) {
          picks.push({ sku: t.sku, lotNumber: lotCode, qty: +take.toFixed(3), uom: t.uom, locationId });
          remain -= take;
        }
      }
  ...
  return (
    <SectionCard title="Disponibilidad y lotes de insumo">
      ...
      <div className="text-xs">
        ...
        {theory.map((line, index) => {
          const proposed = picks.filter(p => p.sku === line.sku).reduce((s, p) => s + p.qty, 0);
          return (
            <div key={`${line.sku}-${index}`} className="grid grid-cols-[1fr,90px,90px] items-start py-0.5">
              ...
              <div className="col-span-3 text-[11px] text-muted-foreground mt-0.5">
                {picks.filter(p => p.sku === line.sku).map((p, pickIndex) => (
                  <span key={`${p.sku}-${p.lotNumber}-${pickIndex}`} className="inline-block mr-1 mb-1 px-1.5 py-0.5 rounded border bg-background">
