...
const statusVariantMap: Record<ProductionStatus, 'default' | 'secondary' | 'success' | 'destructive'> = {
    PLANNED: 'secondary',
    IN_PROGRESS: 'default', // 'default' suele ser el color primario/acento.
    PAUSED: 'secondary',
    DONE: 'success',
    CANCELLED: 'destructive',
    RELEASED: 'success', // Entrada añadida
    QC_HOLD: 'secondary',  // Entrada añadida
};
        ...
        <div className="p-2 max-h-60 overflow-y-auto">
          {recipes.map(bom => (
            <button key={bom.id} onClick={() => onSelectBom(bom)} className="w-full text-left p-2 rounded-lg hover:bg-secondary">
              ...
              <p className="text-xs text-muted-foreground">{bom.outputItemId}</p>
            </button>
          ))}
        </div>
      </SBCard>
        ...
        <div className="p-2 max-h-60 overflow-y-auto">
          {activeOrders.map(order => (
            <button key={order.id} onClick={() => onSelectOrder(order)} className="w-full text-left p-2 rounded-lg hover:bg-secondary">
              ...
              <p className="text-xs text-muted-foreground">{order.outputItemId}</p>
            </button>
          ))}
        </div>
      </SBCard>
