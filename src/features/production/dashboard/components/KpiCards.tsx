...
export function KpiCards({ kpis }: { kpis: any }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <KPI icon={Package} label="Unidades Producidas (30d)" value={kpis.totalUnits30.toLocaleString('es-ES')} color={SB_COLORS.primary.aqua} />
        <KPI icon={BarChart} label="Rendimiento Medio (30d)" value={`${kpis.avgYield30.toFixed(1)}`} unit="%" color="#8b5cf6" />
        <KPI icon={Droplets} label="Coste/Unidad Medio (30d)" value={`${kpis.avgCostUnit30.toFixed(3)}`} unit="€" color="#db2777" />
