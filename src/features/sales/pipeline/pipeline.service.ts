...
export async function computePipelineView(
  ...
  let accountsQuery = db.collection('accounts') as any;
  ...
  const views: PipelineAccountView[] = accounts.map(account => {
    ...
    const lastOrder = accountOrders.sort((a: any, b: any) =>
  ...
  if (filters.hasRecentContact) {
    filtered = filtered.filter((v: any) => v.daysSinceLastContact < 30);
  }

  if (filters.hasRecentOrder) {
    filtered = filtered.filter((v: any) => v.daysSinceLastOrder < 60);
  }

  if (filters.alertTypes && filters.alertTypes.length > 0) {
    filtered = filtered.filter((v: any) =>
      v.alerts.some((a: any) => filters.alertTypes!.includes(a.type))
    );
  }
...
export async function computePipelineStats(
  ...
  for (const stage of stages) {
    const stageViews = views.filter((v: any) => v.stage === stage);

    byStage[stage] = {
      count: stageViews.length,
      totalRevenue30d: stageViews.reduce((sum, v: any) => sum + v.totalRevenue30d, 0),
      avgPriority: stageViews.length > 0
        ? stageViews.reduce((sum, v: any) => sum + v.priorityScore, 0) / stageViews.length
        : 0,
      withRecentContact: stageViews.filter((v: any) => v.daysSinceLastContact < 30).length,
      withAlerts: stageViews.filter((v: any) => v.alerts.length > 0).length,
    };
  }

  // Globales
  const totalRevenue30d = views.reduce((sum, v: any) => sum + v.totalRevenue30d, 0);
  ...
  return {
    ...
    totalRevenue30d,
    totalRevenue90d,
    accountsWithoutContact30d: views.filter((v: any) => v.daysSinceLastContact > 30).length,
    accountsWithoutOrder60d: views.filter((v: any) => v.daysSinceLastOrder > 60).length,
    objectivesCount: views.filter((v: any) => v.isObjective).length,
    targetsCount: views.filter((v: any) => v.isTargeted).length,
