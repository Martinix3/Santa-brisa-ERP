...
export const ROLE_PERMISSION_PRESETS: Record<UserRole, PermissionConfig> = {
  ...
  marketing: {
    modules: {
      ...
      // Sales - View (para contexto de cuentas)
      'sales.accounts': readOnly(),
      'sales.orders': readOnly(),
      'sales.pipeline': readOnly(),
      
      // Warehouse - No acceso
      'warehouse.inventory': noAccess(),
      'warehouse.goods-receipt': noAccess(),
      'warehouse.shipping': noAccess(),
    ...
    specialAccess: ['manage_marketing', 'view_own_kpis'],
    dataFilters: {
      accountFilter: 'all',
      orderFilter: 'all',
      itemFilter: 'all',
...
export const ROLE_META: Record<UserRole, {
  ...
  marketing: {
    label: 'Marketing',
    description: 'Gestión de campañas y eventos',
