...
function assertHasAllKeys<K extends string>(
  ...
  const missing = expected.filter((k: string) => !(k in record));
  const extra = (Object.keys(record as object) as Array<string | number | symbol>)
    .filter((k): k is string | number | symbol => !expected.includes(k as K));
...
function auditCollections() {
  ...
  assertHasAllKeys(
    Object.fromEntries(SANTA_DATA_COLLECTIONS.map((k: string) => [k, true])) as Record<SantaDataKeys, true>,
...
function auditMeta() {
  ...
  // LOT_QC_META: verificamos campos mínimos
