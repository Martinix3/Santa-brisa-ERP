...
export async function processSantaBrainInput(
  ...
  try {
    ...
    console.error('[SantaBrain] Error:', error);
    
    // Si el circuit breaker está abierto o hay error, devolver fallback
    if (error.message?.includes('Circuit breaker')) {
      throw new Error('El servicio de IA está temporalmente no disponible. Intenta de nuevo en unos momentos.');
    }
